...
setupRWFromFile(SDL_RWops* rwops, const char *file, const char *mode)
{
    ...
    if (*file == '/') {
        ...
        if (fp) {
            return setupRWFromFP(rwops, fp, 1);
        }
    } else {
...
SDL_RWops *
setupRWFromMemory(SDL_RWops* rwops, void *mem, int size)
{
    if (!mem) {
...
SDL_RWops *
setupRWFromConstMemory(SDL_RWops* rwops, const void *mem, int size)
{
    if (!mem) {
      SDL_InvalidParamError("mem");
      return (rwops);
