...
VP8Decoder* VP8New(void* (* const alloc)(const size_t, void* const), void* const allocator) {
  VP8Decoder* const dec = (VP8Decoder*)alloc(sizeof(*dec), allocator);
  if (dec != NULL) {
    SetOk(dec);
...
static int ParseFrame(VP8Decoder* const dec,
                      VP8Io* io,
  ...
  for (dec->mb_y_ = 0; dec->mb_y_ < dec->br_mb_y_; ++dec->mb_y_) {
    ...
    if (!VP8ProcessRow(dec, io, alloc, allocator)) {
...
// Main entry point
int VP8Decode(VP8Decoder* const dec, VP8Io* const io, void* (* const alloc)(const size_t, void* const), void* const allocator) {
  int ok = 0;
  if (dec == NULL) {
    return 0;
  }
  ...
  if (ok) {   // good to go.
    ...
    if (ok) ok = VP8InitFrame(dec, io, alloc, allocator);

    // Main decoding loop
    if (ok) ok = ParseFrame(dec, io, alloc, allocator);
