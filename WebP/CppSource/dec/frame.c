...
static int FinishRow(VP8Decoder* const dec,
                     VP8Io* const io,
  ...
  if (io->put != NULL) {
    ...
    if (dec->alpha_data_ != NULL && y_start < y_end) {
      ...
      // good idea.
      io->a = VP8DecompressAlphaRows(dec, y_start, y_end - y_start, alloc, allocator);
      if (io->a == NULL) {
...
int VP8ProcessRow(VP8Decoder* const dec,
                  VP8Io* const io,
                  void* (* const alloc)(const size_t, void* const),
                  void* const allocator) {
  int ok = 1;
  ...
  if (!dec->use_threads_) {
    ...
    ok = FinishRow(dec, io, alloc, allocator);
  } else {
...
static int AllocateMemory(VP8Decoder* const dec, void* (* const alloc)(const size_t, void* const), void* const allocator) {
  ...
  if (needed > dec->mem_size_) {
    dec->mem_size_ = 0;
    dec->mem_ = alloc(needed * sizeof(uint8_t), allocator);
    if (dec->mem_ == NULL) {
      return VP8SetError(dec, VP8_STATUS_OUT_OF_MEMORY,
...
int VP8InitFrame(VP8Decoder* const dec, VP8Io* io, void* (* const alloc)(const size_t, void* const), void* const allocator) {
  if (!InitThreadContext(dec)) return 0;  // call first. Sets dec->num_caches_.
  if (!AllocateMemory(dec, alloc, allocator)) return 0;
  InitIo(dec, io);
