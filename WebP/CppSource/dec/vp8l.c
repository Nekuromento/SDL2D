...
static int DecodeImageStream(int xsize, int ysize,
                             ...
                             void* (* const alloc)(const size_t, void* const),
...
static int ReadHuffmanCodeLengths(VP8LDecoder* const dec,
                                  const int* const code_length_code_lengths, int num_symbols,
  ...
  if (!HuffmanTreeBuildImplicit(&tree, code_length_code_lengths,
                                NUM_CODE_LENGTH_CODES, alloc, allocator)) {
...
static int ReadHuffmanCode(int alphabet_size,
  ...
  if (simple_code) {  // Read symbols, codes & code lengths directly.
    ...
    ok = HuffmanTreeBuildExplicit(tree, code_lengths, codes, symbols,
                                  alphabet_size, num_symbols, alloc, allocator);
    ...
    code_lengths =
        (int*)alloc((uint64_t)alphabet_size * sizeof(*code_lengths), allocator);
    memset(code_lengths, 0, (uint64_t)alphabet_size * sizeof(*code_lengths));
    ...
    ok = ReadHuffmanCodeLengths(dec, code_length_code_lengths, alphabet_size,
                                code_lengths, alloc, allocator);
    if (ok) {
      ok = HuffmanTreeBuildImplicit(tree, code_lengths, alphabet_size, alloc, allocator);
    }
  }
  ok = ok && !br->error_;
  if (!ok) {
...
static int ReadHuffmanCodes(VP8LDecoder* const dec,
                            int xsize,
                            int ysize,
  ...
  htree_groups =
      (HTreeGroup*)alloc((uint64_t)num_htree_groups * sizeof(*htree_groups), allocator);
  memset(htree_groups, 0, (uint64_t)num_htree_groups * sizeof(*htree_groups));
  if (htree_groups == NULL) {
    dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
    goto Error;
  }
  ...
  for (i = 0; i < num_htree_groups; ++i) {
    ...
    for (j = 0; j < HUFFMAN_CODES_PER_META_CODE; ++j) {
      ...
      if (!ReadHuffmanCode(alphabet_size, dec, htrees + j, alloc, allocator)) goto Error;
    }
  }
...
static int AllocateAndInitRescaler(VP8LDecoder* const dec,
                                   VP8Io* const io,
                                   void* (* const alloc)(const size_t, void* const),
                                   void* const allocator) {
  ...
  uint8_t* memory = (uint8_t*)alloc(memory_size * sizeof(*memory), allocator);
  memset(memory, 0, memory_size * sizeof(*memory));
  if (memory == NULL) {
    dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
    return 0;
  }
...
static int ExpandColorMap(int num_colors,
  ...
  uint32_t* const new_color_map =
      (uint32_t*)alloc((uint64_t)final_num_colors * sizeof(*new_color_map), allocator);
...
static int ReadTransform(int* const xsize,
                         int const* ysize,
                         VP8LDecoder* const dec,
                         void* (* const alloc)(const size_t, void* const),
                         void* const allocator) {
  ...
  switch (type) {
    ...
    case CROSS_COLOR_TRANSFORM:
      ...
      ok = DecodeImageStream(VP8LSubSampleSize(transform->xsize_,
                             ...
                             VP8LSubSampleSize(transform->ysize_,
                                               transform->bits_),
                             0, dec, &transform->data_, alloc, allocator);
      break;
    case COLOR_INDEXING_TRANSFORM: {
       ...
       *xsize = VP8LSubSampleSize(transform->xsize_, bits);
       ...
       ok = DecodeImageStream(num_colors, 1, 0, dec, &transform->data_, alloc, allocator);
       ok = ok && ExpandColorMap(num_colors, transform, alloc, allocator);
      break;
    }
    case SUBTRACT_GREEN:
      break;
...
VP8LDecoder* VP8LNew(void* (* const alloc)(const size_t, void* const), void* const allocator) {
  VP8LDecoder* const dec = (VP8LDecoder*)alloc(sizeof(*dec), allocator);
...
static int DecodeImageStream(int xsize, int ysize,
  ...
  if (is_level0) {
    while (ok && VP8LReadBits(br, 1)) {
      ok = ReadTransform(&transform_xsize, &transform_ysize, dec, alloc, allocator);
    }
  }
  ...
  ok = ok && ReadHuffmanCodes(dec, transform_xsize, transform_ysize,
                              color_cache_bits, is_level0, alloc, allocator);
  if (!ok) {
    dec->status_ = VP8_STATUS_BITSTREAM_ERROR;
    goto End;
  }
  ...
  if (color_cache_bits > 0) {
    ...
    if (!VP8LColorCacheInit(&hdr->color_cache_, color_cache_bits, alloc, allocator)) {
      dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
      ok = 0;
      goto End;
    }
...

  {
    ...
    data = (uint32_t*)alloc(total_size * sizeof(*data), allocator);
    if (data == NULL) {
      dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
      ok = 0;
      goto End;
...
static int AllocateInternalBuffers32b(VP8LDecoder* const dec,
                                      int final_width,
                                      void* (* const alloc)(const size_t, void* const),
                                      void* const allocator) {
  ...
  dec->pixels_ = (uint32_t*)alloc(total_num_pixels * sizeof(uint32_t), allocator);
  if (dec->pixels_ == NULL) {
...
static int AllocateInternalBuffers8b(VP8LDecoder* const dec,
  ...
  dec->argb_cache_ = NULL;    // for sanity check
  dec->pixels_ = (uint32_t*)alloc(total_num_pixels * sizeof(uint8_t), allocator);
  if (dec->pixels_ == NULL) {
    dec->status_ = VP8_STATUS_OUT_OF_MEMORY;
    return 0;
  }
...
int VP8LDecodeAlphaHeader(ALPHDecoder* const alph_dec,
  ...
  alph_dec->vp8l_dec_ = VP8LNew(alloc, allocator);
  ...
  if (!DecodeImageStream(alph_dec->width_, alph_dec->height_, 1, dec, NULL, alloc, allocator)) {
    goto Err;
  }
  ...
  if (dec->next_transform_ == 1 &&
    ...
    ok = AllocateInternalBuffers8b(dec, alloc, allocator);
  } else {
    // Allocate internal buffers (note that dec->width_ may have changed here).
    alph_dec->use_8b_decode = 0;
    ok = AllocateInternalBuffers32b(dec, alph_dec->width_, alloc, allocator);
  }
...
int VP8LDecodeHeader(VP8LDecoder* const dec,
                     VP8Io* const io,
                     void* (* const alloc)(const size_t, void* const),
                     void* const allocator) {
  ...
  dec->action_ = READ_HDR;
  if (!DecodeImageStream(width, height, 1, dec, NULL, alloc, allocator)) goto Error;
  return 1;
...
int VP8LDecodeImage(VP8LDecoder* const dec,
  ...
  if (!AllocateInternalBuffers32b(dec, io->width, alloc, allocator)) goto Err;

  if (io->use_scaling && !AllocateAndInitRescaler(dec, io, alloc, allocator)) goto Err;
