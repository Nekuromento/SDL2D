...
static VP8StatusCode AllocateBuffer(WebPDecBuffer* const buffer, void* (* const allocate)(const size_t, void* const), void* const allocator) {
  ...
  if (!buffer->is_external_memory && buffer->private_memory == NULL) {
    ...
    output = (uint8_t*)allocate(total_size * sizeof(*output), allocator);
    if (output == NULL) {
...
VP8StatusCode WebPAllocateDecBuffer(int w, int h,
  ...
  return AllocateBuffer(out, options->alloc, options->allocator);
}
