...
static VP8StatusCode DecodeInto(const uint8_t* const data, size_t data_size,
  ...
  if (!headers.is_lossless) {
    VP8Decoder* const dec = VP8New(params->options->alloc, params->options->allocator);
    if (dec == NULL) {
    ...
    if (!VP8GetHeaders(dec, &io)) {
      ...
      if (status == VP8_STATUS_OK) {  // Decode
        if (!VP8Decode(dec, &io, params->options->alloc, params->options->allocator)) {
          status = dec->status_;
        }
      }
    }
  } else {
    VP8LDecoder* const dec = VP8LNew(params->options->alloc, params->options->allocator);
    if (dec == NULL) {
      return VP8_STATUS_OUT_OF_MEMORY;
    }
    if (!VP8LDecodeHeader(dec, &io, params->options->alloc, params->options->allocator)) {
      status = dec->status_;   // An error occurred. Grab error status.
    } else {
      // Allocate/check output buffers.
      status = WebPAllocateDecBuffer(io.width, io.height, params->options,
                                     params->output);
      if (status == VP8_STATUS_OK) {  // Decode
        if (!VP8LDecodeImage(dec, params->options->alloc, params->options->allocator)) {
          status = dec->status_;
        }
      }
    }
